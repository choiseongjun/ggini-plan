...
export function SnackLog({onLogged}: {onLogged: () => void}) {
 ...
 async function log() {
  ...
  try {
   const r = await fetch('/api/food-intake', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({action: 'log', id: crypto.randomUUID(), version: 0, referenceCode: picked.code, portions})});
   const d = await r.json();
   if (!r.ok) throw new Error(d.error);
   setDone(`${picked.name} 기록했어요${picked.kcal !== null ? ` · 약 ${n(picked.kcal * portions)}kcal` : ''}`);
