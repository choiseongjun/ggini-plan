...
export function useHomePlan(userId?:string){
 ...
 useEffect(()=>{
  ...
  cachedJson('/api/shopping-plan',{key:`/api/shopping-plan|${userId??'guest'}`,ttl:10*60_000}).then(d=>{
   ...
   setPlan(conditions&&ids.length?{conditions,ids,products:d.baseProducts??d.products}:null);setError('');
