...
export default function CollectionPage() {
  ...
  async function read(item: {id:string; name:string}, imageIndex?: number): Promise<Result> {
    try {const r = await fetch('/api/admin/catalog/nutrition', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:item.id,imageIndex})}); const data = await r.json(); if(!r.ok)throw new Error(data.error); return {...data,id:item.id,name:item.name};}
