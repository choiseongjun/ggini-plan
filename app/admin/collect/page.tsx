...
type Result = {id: string; name: string; extracted?: ExtractedNutrition; text?: string; warning?: string; sourceUrl?: string; images?: string[]; imageUrl?: string; version?: string; error?: string; saved?: boolean; estimate?:CatalogItem['nutritionEstimate'];checkedImages?:number; fillNotice?:string};
export default function CollectionPage() {
  ...
  async function save(result: Result) {
    ...
    try {const r=await fetch('/api/admin/catalog/nutrition',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:result.id,extracted:result.extracted,sourceUrl:result.sourceUrl,version:result.version,estimate:result.estimate})});const data=await r.json();if(!r.ok)throw new Error(data.error);setResults(prev=>prev.map(v=>v.id===result.id?{...v,saved:true}:v));setAttempt(v=>v+1);}
  ...
  return <main className="admin-shell"><header className="admin-header"><div><span className="admin-kicker">KKINIPLAN · CONTENT MANAGER</span><h1>상품 수집</h1><p>판매 상품을 등록하고, 원문 영양정보를 읽어 확인 후 저장해요.</p></div><Link href="/">앱으로 돌아가기 ↗</Link></header>
    ...
    <div className="nutrition-results">{results.map(result=><article key={result.id}><h3>{result.name}</h3>{result.sourceUrl&&<a href={result.sourceUrl} target="_blank" rel="noopener noreferrer">판매처 원문 확인 ↗</a>}{result.error&&<p role="alert">{result.error}</p>}{result.fillNotice&&<p role="status">{result.fillNotice}{result.checkedImages?` (사진 ${result.checkedImages}장 확인)`:''}</p>}{result.estimate&&<p><strong>추정 영양정보</strong> · {result.estimate.note}</p>}{result.warning&&<p>{result.warning}</p>}
      ...
      <details><summary>인식 원문 보기</summary><pre>{result.text||'읽을 수 있는 영양정보 원문이 없습니다.'}</pre></details><button disabled={busy||result.saved||!result.extracted.nutritionBasis?.trim()||fields.some(([key])=>result.extracted![key]===null)} onClick={()=>save(result)}>{result.saved?'저장 완료':result.estimate?'추정 표시와 함께 저장':'확인한 영양성분 저장'}</button></>}
