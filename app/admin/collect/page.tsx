...
type Result = {id: string; name: string; extracted?: ExtractedNutrition; text?: string; warning?: string; sourceUrl?: string; images?: string[]; imageUrl?: string; version?: string; error?: string; saved?: boolean};
export default function CollectionPage() {
  ...
  async function read(item: {id:string; name:string}, imageIndex?: number): Promise<Result> {
    try {const r = await fetch('/api/admin/catalog/nutrition', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:item.id,imageIndex})}); const data = await r.json(); if(!r.ok){if(r.status===502||r.status===401||r.status===403)stop.current=true;throw new Error(data.error);} return {...data,id:item.id,name:item.name};}
  ...
  return <main className="admin-shell"><header className="admin-header"><div><span className="admin-kicker">KKINIPLAN · CONTENT MANAGER</span><h1>상품 수집</h1><p>판매 상품을 등록하고, 원문 영양정보를 읽어 확인 후 저장해요.</p></div><Link href="/">앱으로 돌아가기 ↗</Link></header>
    ...
    <NutritionAIStatus/>
    <section className="oasis-collection"><h2>영양성분 수집</h2><small>등록된 한국 상품 중 영양 수치가 비어 있는 상품을 확인합니다. 컬리·오아시스의 영양정보 원문과 표시사항 사진을 읽어요. 화면을 유지해 주세요. 표가 없거나 인식이 어려운 상품은 직접 입력이 필요합니다.</small>
