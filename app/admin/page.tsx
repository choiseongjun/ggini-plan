...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    ...
    {loading ? <div className="admin-message">상품을 불러오는 중입니다…</div> : !draft ? <div className="admin-message"><strong>{error || "상품이 없습니다."}</strong>{error ? <button type="button" onClick={() => { setError(""); setLoading(true); setLoginAttempt(value => value + 1); }}>다시 불러오기</button> : <button type="button" onClick={newItem}>첫 상품 등록하기</button>}</div> : <div className="admin-grid">
      ...
      <form className="admin-form" onSubmit={save}>
        ...
        <section><h3>판매 상품</h3><p>실제 상품 페이지의 상품명·구성·가격을 확인해 입력하세요. 가격은 확인 시점의 참고값입니다.</p><div className="admin-fields">
          ...
          <label className="wide">상품 대표 사진 링크<input type="url" pattern="https://.*" placeholder="판매처 상품 사진의 HTTPS 주소" value={draft.productImageUrl ?? ""} onChange={(e) => change("productImageUrl", e.target.value || null)}/></label>
