...
import {catalogSeller,catalogSellers} from "../../lib/catalog-sellers";
...
export default function AdminPage() {
  ...
  const marketItems=items.filter(item=>(marketFilter==='all'||item.market===marketFilter)&&(sellerFilter==='all'||catalogSeller(item.productUrl)===sellerFilter));
  ...
  const visibleItems = photoItems.filter(item => nutritionFilter === "all" || nutritionStatus(item) === nutritionFilter).sort((a,b)=>{
    if(sortOrder==='name')return a.name.localeCompare(b.name,'ko');
    const time=(s?:string|null)=>s?Date.parse(s)||0:0;
    return (sortOrder==='updated'?time(b.updatedAt)-time(a.updatedAt):time(b.createdAt)-time(a.createdAt))||time(b.updatedAt)-time(a.updatedAt)||a.id.localeCompare(b.id);
  });
  const [pageSelection,setPageSelection]=useState({filter:'',page:1});
  const filterKey=JSON.stringify([marketFilter,sellerFilter,query,categoryFilter,foodTypeFilter,photoFilter,nutritionFilter,sortOrder]);
  ...
  useEffect(() => {
    fetch("/api/admin/catalog", { cache: "no-store" })
      ...
      .then((data) => { if (!data) return; setLoginRequired(false); setItems(data); const requested = new URLSearchParams(window.location.search).get("item"); const first = data.find(item => item.id === requested) ?? data[0]; setSelectedId(first?.id ?? ""); setDraft(first ?? null); })
  ...
  return <main className="admin-shell">
    ...
    <nav className="admin-collection-tabs" aria-label="상품 관리 메뉴"><Link href="/admin" aria-current="page">상품·영양 관리</Link><Link href="/admin/collect">상품 수집 →</Link><Link href="/admin/deals">핫딜 수집·관리</Link><Link href="/admin/foodsafety">영양DB 조회</Link><Link href="/admin/recipe-optimizer">유사 레시피 생성기</Link></nav>
    {loading ? <AppLoading message="상품 정보를 불러오고 있어요"/> : !draft ? <div className="admin-message"><strong>{error || "상품이 없습니다."}</strong>{error ? <button type="button" onClick={() => { setError(""); setLoading(true); setLoginAttempt(value => value + 1); }}>다시 불러오기</button> : <button type="button" onClick={()=>newItem()}>첫 상품 등록하기</button>}</div> : <div className="admin-grid">
      ...
      <form className="admin-form" onSubmit={save}>
        ...
        <section><h3>판매 상품 · {catalogSellers[catalogSeller(draft.productUrl)]}</h3><p>실제 상품 페이지의 상품명·구성·가격을 확인해 입력하세요. 가격은 확인 시점의 참고값입니다.</p><div className="admin-fields">
