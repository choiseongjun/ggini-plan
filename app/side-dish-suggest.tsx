...
export function SideDishSuggest({main, conditions}: {main: PlanProduct; conditions: PlanConditions}) {
 ...
 useEffect(() => {
  let alive = true;
  engine.sides(main.id, conditions).then((list) => { if (alive) { setSides(list); setError(''); } }).catch((e) => { if (alive) setError(e instanceof Error ? e.message : '반찬을 불러오지 못했어요.'); });
  return () => { alive = false; };
 ...
 if (!sides.length) return <p className="sd-note">이 메뉴에 맞는 반찬을 아직 준비하지 못했어요.</p>;
