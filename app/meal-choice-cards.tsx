...
export function MealChoiceCards({ready,recipeName,recipeCost}:{ready:PlanProduct;recipeName:string;recipeCost:number|null}){
 ...
 return <>
  <dl className="cook-price-compare">
   ...
   <div><dt>🍳 직접 만들기</dt><dd>{recipeCost===null?'가격 미확인':`약 ${won(Math.round(recipeCost/100)*100)}`}</dd><span className="cook-choice-name">{recipeName}</span><small>기본 레시피 1인분 · 사용 재료비</small><small>{recipeCost===null?'일부 재료 가격 미확인':'재료를 통째로 사는 금액은 별도'}</small></div>
  </dl>
