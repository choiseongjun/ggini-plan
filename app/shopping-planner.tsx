...
import { basket, basketTotal, candidates, initialConditions, parseConditions, recommendShopping, swapMeal, type PlanConditions, type PlanProduct } from '../lib/shopping-plan';
import './shopping-planner.css';

const won=(n:number)=>`${n.toLocaleString('ko-KR')}원`;
const draftKey='kkiniplan-shopping-draft-v1';
export function ShoppingPlanner({userId,onLogin}:{userId?:string;onLogin:()=>void}){
 ...
 useEffect(()=>{
  ...
  fetch('/api/shopping-plan',{cache:'no-store',signal:controller.signal}).then(async r=>{const d=await r.json();if(!r.ok)throw new Error(d.error);return d;}).then(d=>{
   ...
   try{const draft=JSON.parse(sessionStorage.getItem(draftKey)??'null');const c=parseConditions(draft?.conditions);
    if(c){setConditions(c);if(Array.isArray(draft.mealIds)&&draft.mealIds.length===c.meals&&draft.mealIds.every((id:string)=>candidates(d.products,c).some(p=>p.id===id))&&basketTotal(draft.mealIds,d.products,c.owned)<=c.budget)setIds(draft.mealIds);}
 ...
 function update(patch:Partial<PlanConditions>){const c={...conditions,...patch};setConditions(c);setIds([]);setMessage('');setError('');remember(c,[]);}
 function generate(){
  setMessage('');setError('');const c=parseConditions(conditions);
  if(!c){setError('예산은 1,000~1,000,000원 사이로 입력해 주세요.');return;}
 ...
 async function restore(){
  ...
  try{const r=await fetch('/api/shopping-plan?saved=1',{cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error);if(!d.plan){setMessage('아직 저장한 장보기 식단이 없어요.');return;}
   ...
   const valid=Array.isArray(d.plan.mealIds)&&d.plan.mealIds.length===c.meals&&d.plan.mealIds.every((id:string)=>candidates(products,c).some(p=>p.id===id))&&basketTotal(d.plan.mealIds,products,c.owned)<=c.budget;
 ...
 return <section className="shopping-planner" aria-labelledby="planner-title">
  <div className="planner-heading"><span>이번 주, 뭐 먹지?</span><h2 id="planner-title">예산 안에서<br/><em>식단부터 장보기까지</em></h2><p>조건을 고르면 실제 판매 상품으로 끼니와 구매 수량을 함께 정해요. 로그인 없이 시작하세요.</p></div>
  <form className="planner-form" onSubmit={e=>{e.preventDefault();generate();}}>
   <label>준비할 끼니<select value={conditions.meals} onChange={e=>update({meals:Number(e.target.value)})}><option value={5}>5끼 · 평일 한 끼씩</option><option value={7}>7끼 · 일주일 한 끼씩</option><option value={10}>10끼 · 평일 두 끼씩</option><option value={14}>14끼 · 일주일 두 끼씩</option></select></label>
   ...
   <button className="primary-button" disabled={loading||busy||!products.length}>{loading?'판매 상품 불러오는 중…':'내 예산으로 식단 추천받기 →'}</button>
  </form>
  ...
  {!!ids.length&&<div className="planner-result">
   ...
   <h3>이렇게 먹어요</h3><p className="body-note">밥·면 중심의 메뉴 구성 제안이에요. 조리 기기·시간은 상품 페이지를 확인하고, 식사량에 따라 양을 조절해 주세요.</p>
   <ol className="planner-meals">{ids.map((id,i)=>{const p=products.find(p=>p.id===id)!;return <li key={`${i}-${id}`}><ProductThumb item={p}/><div><small>{Math.floor(i/(conditions.meals>=10?2:1))+1}일차 · {conditions.meals>=10?(i%2?'저녁':'점심'):'한 끼'}</small><strong>{p.name}</strong><small>{p.servingNote} · 1회분 사용</small></div><button type="button" onClick={()=>swap(i)} aria-label={`${i+1}번째 끼니 다른 메뉴`}>교체</button></li>;})}</ol>
   <h3>살 것 · 판매 묶음 기준</h3><p className="body-note">‘이미 있어요’는 이 식단에 필요한 수량을 모두 가지고 있을 때 선택하세요.</p>
   <div className="planner-basket">{rows.map(r=><article key={r.product.id}><div><strong>{r.product.name}</strong><span>{r.product.detail} × {r.packs}묶음</span><span>{r.uses}회분 사용 · 계획 후 {r.left}회분 남음</span><b>{r.have?'보유 수량 사용':won(r.cost)}</b></div><label><input type="checkbox" checked={r.have} onChange={()=>own(r.product.id)}/>이미 있어요</label><a href={r.product.productUrl!} target="_blank" rel="noopener noreferrer">상품·조리법 확인 ↗</a></article>)}</div>
