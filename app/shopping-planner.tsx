...
import {cookingEfforts} from '../lib/cooking-effort';
import {CookingEffortPicker} from './cooking-effort-picker';
...
import {PlannerFab,fabIcons} from './planner-fab';
...
const taiwanConditions:PlanConditions={...initialConditions,cookingEffort:undefined,mealMode:'ready',budget:60000};
...
const planTtl=10*60_000;
// 내 정보(신체 정보·목표·제외 재료)는 모드와 상관없이 서버에서 항상 추천에 반영된다. 모든 모드에 같은 한 줄로 알려 준다.
...
export function ShoppingPlanner({userId,onLogin,mode='plan',dashboard}:{userId?:string;onLogin:()=>void;mode?:'plan'|'cart'|'settings';dashboard?:DashboardData|null}){
 ...
 // 예산 안내(최저 구성 탐색)는 예산을 입력받는 대만판에서만 계산한다.
 const {guide:budgetGuide,pending:budgetPending}=useBudgetGuide(locale.isTaiwan?products:NO_PRODUCTS,guideKey);
 const [mealCap,setMealCap]=useState<number|null>(null);
 const [showSetup,setShowSetup]=useState(false);
 const waitingForBudget=!locale.isTaiwan&&automaticBudget&&budgetPending;
 // 메뉴가 전부 원재료 레시피라 먹는 방식·반찬 개수 선택은 없앴다. 예전에 '밀키트'·'반찬 2개'로 저장한 사람도
 // 후보가 0개가 되지 않게 항상 직접 요리·한 그릇(레시피에 김치·나물 반찬 포함) 기준으로 계산한다.
 // 한국판 메뉴는 전부 '메인 요리 + 밥'이라 음식 종류(밥류·빵류…) 제한은 대부분의 메인을 빼 버린다 — 예전에 저장한 선택도 무시한다.
 const conditions:PlanConditions={...budgetConditions,...(locale.isTaiwan?{}:{mealKinds:[],budgetMode:'balanced' as const}),mealMode:'cook',cooking:'all',sideCount:0,budget:locale.isTaiwan?budgetConditions.budget:mealCap?Math.min(NO_BUDGET_CAP,Math.max(1000,Math.round(mealCap*budgetConditions.meals*(budgetConditions.people??1)))):NO_BUDGET_CAP};
 ...
 function updatePreferences(patch:Partial<Pick<PlanConditions,'mealKinds'|'goal'|'budgetMode'|'swapPreferences'|'cookingEffort'>>){
 ...
 async function save(){
  ...
  setBusy(true);setError('');setMessage('');
   try{const r=await mutate({method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({conditions,mealIds:ids})});const d=await r.json();if(!r.ok)throw new Error(d.error);sessionStorage.removeItem('ggini-pending-adoption');setMessage('식단을 계정에 저장했어요. 준비한 뒤 먹었어요를 누르면 식비와 영양 기록이 쌓여요.');}
 ...
 const personalRecommendation=!locale.isTaiwan&&!userId&&<div className="personal-recommendation-entry"><button type="button" disabled={loading||busy} onClick={()=>{remember(conditions,ids);clearRecordMode();rememberProfileIntent();onLogin();}}>로그인하고 내 정보로 맞춤 추천</button><small>키·체중·활동량과 식사 취향을 입력하면 더 잘 맞춰드려요.</small></div>;
 // This explicit return from profile setup requests a fresh personalized plan once.
 useEffect(()=>{
  if(loading||!progress.ready||!catalogReady||!userId||mode!=='plan'||locale.isTaiwan)return;
  const url=new URL(window.location.href);if(url.searchParams.get('recommend')!=='profile')return;
  const frame=requestAnimationFrame(()=>{url.searchParams.delete('recommend');window.history.replaceState(null,'',url.pathname+url.search+url.hash);void generate();});
  return()=>cancelAnimationFrame(frame);
  // The one-shot URL intent is consumed only after bootstrap finishes.
 ...
 return locale.render(<PlanEngineContext.Provider value={engine}><section onClickCapture={e=>{if(locale.isTaiwan)return;const a=(e.target as Element).closest('a');if(a&&products.some(p=>p.productUrl===a.href||p.recipe?.ingredients.some(i=>i.product.productUrl===a.href)))trackPlanner('seller');}} ref={plannerRef} id={mode==='settings'?'shopping-settings':undefined} className={`shopping-planner${mode==='plan'?' home-planner':''}${!ids.length?' planner-empty':''}`} aria-labelledby="planner-title">
  ...
  {mode==='plan'&&!ids.length&&!locale.isTaiwan&&!showSetup&&<section className="home-start" aria-labelledby="planner-title">
   ...
   <CookingEffortPicker value={conditions.cookingEffort} onChange={cookingEffort=>updatePreferences({cookingEffort})} disabled={loading||busy}/>
   {mealTimesPicker}
  ...
  {mode!=='cart'&&<form className="planner-form" onSubmit={e=>{e.preventDefault();if(waitingForBudget)return;if(mode==='settings')void savePreferences();else void generate();}}>
   ...
   {!locale.isTaiwan&&<CookingEffortPicker value={conditions.cookingEffort} onChange={cookingEffort=>updatePreferences({cookingEffort})} disabled={loading||busy}/>}
   ...
   <small>밖에서 먹는 끼니는 선택하지 마세요.</small>
   </>}
   {locale.isTaiwan&&<details className="planner-more-options" open={moreOptions} onToggle={e=>setMoreOptions(e.currentTarget.open)}><summary>更多設定・預算與排除食材</summary>
  {personalization&&<details className="planner-profile-summary"><summary>내 정보 반영 내용</summary><div className="meal-notice">{personalization.blocked?<p>현재 신체 정보에서는 자동 맞춤 추천을 제공하지 않아요.</p>:personalization.hasProfile?<><strong>내 정보 기준 · 하루 유지 필요량 약 {personalization.dailyCalories?.toLocaleString()} kcal</strong><p>하루 {personalization.meals}끼 기준 한 끼 약 {personalization.perMealCalories} kcal와 가까운 상품을 우선해요. 선택한 끼니만 추천하며 하루 전체 영양을 충족하는 식단은 아니에요.</p><small>{personalization.nutritionMatched===null?'추천할 때 등록된 영양정보를 내 필요 열량과 비교해요.':personalization.nutritionMatched?`등록 영양값을 비교할 수 있는 상품 ${personalization.nutritionMatched}개`:'현재 상품은 영양 정보가 부족해 열량 기준의 비교가 어려워요. 등록된 기준량과 1회분 중량이 있어야 영양값을 비교할 수 있어요.'}</small></>:<p><Link href="/profile#profile-settings">신체 정보를 입력하면 내 필요 열량을 기준으로 추천받을 수 있어요 →</Link></p>}{personalization.style&&<p>식단 취향: {personalization.style} · 제외 재료: {(conditions.excluded??[]).map(key=>excludedFoods[key]).join(', ')||'없음'}</p>}</div></details>}

   {locale.isTaiwan&&!loading&&progress.ready&&<div className="planner-budget-hint" aria-live="polite">
   ...
   {locale.isTaiwan&&!loading&&progress.ready&&budgetGuide&&<details className="planner-budget-guide" open={budgetGuide.minimum===null||conditions.budget<budgetGuide.minimum}><summary>내 예산으로 얼마나 준비할 수 있나요?</summary>
