 ...
 try {
  ...
  const results = await Promise.all([
   db.query(`SELECT p.id::text,p.alias,p.title,p.body,p.items,p.style,p.created_at AS "createdAt",COALESCE(p.user_id=$1,false) AS mine,
     ...
     FROM community_posts p ORDER BY p.created_at DESC,p.id DESC LIMIT 30`,[uid]),
  ...
  return json({posts:results[0].rows,challenges:results[1].rows,basket:results[2].rows[0]??null,week:results[3].rows[0].week});
...
export async function POST(request:NextRequest) {
 ...
 try {
  ...
  const db=getPool();
  if(p.action === "share") {
   if(!validText(p.alias,24)||!validText(p.title,80)||!validText(p.body,1000)||typeof p.style!=="string"||!Object.hasOwn(dietStyles,p.style)||!Array.isArray(p.items)||p.items.length<1||p.items.length>20||p.items.some((x:unknown)=>typeof x!=="string")) return authFailure("별명, 제목, 후기와 공유할 재료를 확인해 주세요.",400);
   const catalog=await catalogItems(); const ids=new Set(p.items); const items=catalog.filter(i=>ids.has(i.id)).map(({id,name,price,emoji,detail})=>({id,name,price,emoji,detail}));
   if(items.length!==ids.size) return authFailure("선택한 상품을 확인해 주세요.",400);
   const row=await db.query(`INSERT INTO community_posts(user_id,alias,title,body,items,style) VALUES($1,$2,$3,$4,$5,$6) RETURNING id::text`,[user.id,p.alias.trim(),p.title.trim(),p.body.trim(),JSON.stringify(items),p.style]);
  ...
  if(p.action === "like" || p.action === "tip" || p.action === "delete-post" || p.action === "adopt") {
   ...
   if(!Number.isInteger(p.budget)||p.budget<1000||p.budget>10000000) return authFailure("장보기 예산을 확인해 주세요.",400);
   ...
   for(const item of candidates) if(total+item.price<=p.budget){items.push({id:item.id,name:item.name,price:item.price,emoji:item.emoji,detail:item.detail});total+=item.price;}
   if(!items.length) return authFailure("예산과 제외 재료를 반영하면 담을 상품이 없어요. 예산을 바꾸거나 다른 장바구니를 골라 주세요.",422);
   const client=await db.connect();
   try {
    await client.query("BEGIN");
    await client.query(`INSERT INTO community_baskets(user_id,source_post_id,items,budget) VALUES($1,$2,$3,$4) ON CONFLICT(user_id) DO UPDATE SET source_post_id=EXCLUDED.source_post_id,items=EXCLUDED.items,budget=EXCLUDED.budget,updated_at=NOW()`,[user.id,p.postId,JSON.stringify(items),p.budget]);
