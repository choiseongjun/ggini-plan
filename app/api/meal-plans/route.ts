...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await getPool().query(`SELECT id::text, profile_snapshot AS profile, diet_snapshot AS diet,
      nutrition_target_snapshot AS "nutritionTarget", recommendation, variant, created_at AS "createdAt" FROM meal_plans
...
export async function POST(request: NextRequest) {
  ...
  const nutritionTarget = input?.nutritionTarget == null ? null : parseNutritionTarget(input.nutritionTarget);
  if (input?.nutritionTarget != null && !nutritionTarget) return authFailure("칼로리·탄단지 목표를 확인해 주세요.", 400);
  try {
  const catalog = await catalogItems();
  const recommendation = recommendMeals(profile, diet, variant, catalog, [], nutritionTarget);
  if (!recommendation) return authFailure("선택한 시간대와 제외 재료 조건에 맞는 식단이 부족해요. 식사 시간·제외 재료 설정을 확인해 주세요. 임신·수유 중에는 자동 추천을 제공하지 않아요.", 422);
    const user = await sessionUser(request);
    const plan = { profile, diet, recommendation, variant, nutritionTarget };
