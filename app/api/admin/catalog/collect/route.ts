...
export async function POST(r:NextRequest){
 ...
 try{return Response.json(await collectCatalog(input),{headers:{'Cache-Control':'no-store'}});}catch(e){return Response.json({error:e instanceof Error&&/수집|판매처/.test(e.message)?e.message:'상품 수집을 완료하지 못했어요. 다시 실행해 주세요.'},{status:503});}
}
