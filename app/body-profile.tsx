  ...
  return <>
    ...
    {showPlan && <section className="personal-meals" aria-label="맞춤 식단 추천" aria-live="polite">
      ...
      {recommendation ? <>
        ...
        {recommendation.meals.map((meal,i)=><article className="personal-meal-card" key={`${i}-${meal.name}`}><div className="personal-meal-head"><span className="personal-meal-emoji" aria-hidden="true">{meal.emoji}</span><div><span>{i+1}번째 끼니 · {meal.time}</span><h4>{meal.name}</h4></div><b>{meal.kcal}<small>kcal</small></b></div><div className="personal-macros">단백질 {meal.protein}g <i/> 탄수화물 {meal.carbs}g <i/> 지방 {meal.fat}g</div><details><summary>재료 분량과 간단 조리법</summary><ul>{meal.ingredients.map(item=><li key={item.food}><span>{item.name}{item.product?.url && <a className="ingredient-product" href={item.product.url} target="_blank" rel="noopener noreferrer">{item.product.name} ↗</a>}</span><b>{item.grams}g</b></li>)}</ul><p>{meal.tip}</p></details></article>)}
