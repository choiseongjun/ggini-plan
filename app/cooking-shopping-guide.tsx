...
export function CookingShoppingGuide({ids,products,conditions,onOwn,disabled}:{ids:string[];products:PlanProduct[];conditions:PlanConditions;onOwn:(id:string)=>void;disabled:boolean}){
 ...
 return <section className="cooking-shopping-guide" aria-label="장본 재료로 요리하기">
  ...
  <div className="cooking-menu-grid">{meals.map((p,i)=><article key={i}><div className="cooking-day-badge">{schedule[i].day}일차 · {slotLabels[schedule[i].slot]} <span>{p.recipe?`약 ${p.recipe.minutes}분 · `:''}{conditions.people??1}인분</span></div><h4>{p.name}</h4>{p.recipe?<><h5>🧺 준비할 재료 · 전체 분량</h5><ul className="cooking-ingredient-chips">{p.recipe.ingredients.map(part=><li key={part.product.id}><span>{part.product.name}</span><strong>{amount(part.product,part.packs*(conditions.people??1))}</strong></li>)}</ul><h5>🍳 만드는 순서</h5><ol>{p.recipe.steps.map((step,j)=><li key={j}><span aria-hidden="true">{j+1}</span><p>{step}</p></li>)}</ol>{!p.recipe.assembly&&<details className="cooking-video-details"><summary>조리 영상 참고하기</summary><RecipeVideos key={p.id} dishId={p.id}/></details>}</>:<p>상품 포장에 표시된 조리법을 따라 준비해 주세요.</p>}</article>)}</div>
