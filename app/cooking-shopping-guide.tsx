...
export function CookingShoppingGuide({ids,products,conditions,onOwn,disabled}:{ids:string[];products:PlanProduct[];conditions:PlanConditions;onOwn:(id:string)=>void;disabled:boolean}){
 ...
 return <section className="cooking-shopping-guide" aria-label="장본 재료로 요리하기">
  ...
  <div className="cooking-menu-grid">{meals.map((p,i)=><article key={i}><small>{schedule[i].day}일차 · {slotLabels[schedule[i].slot]}{p.recipe?` · 약 ${p.recipe.minutes}분`:''}</small><h4>{p.name}</h4>{p.recipe?<><p>{p.recipe.ingredients.map(part=>part.label).join(' · ')}</p><ol>{p.recipe.steps.map((step,j)=><li key={j}>{step}</li>)}</ol></>:<p>상품 포장에 표시된 조리법을 따라 준비해 주세요.</p>}</article>)}</div>
