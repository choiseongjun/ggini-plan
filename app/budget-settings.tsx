...
export function BudgetSettings({data,userId,onLogin,onRefresh,monthlyOnly=false}:{monthlyOnly?:boolean;data:DashboardData|null;userId?:string;onLogin:()=>void;onRefresh:()=>Promise<void>}){
 ...
 return <section className="personal-meal-card" aria-labelledby="food-budget-title">
  ...
  <p className="body-note">{monthlyOnly?'외식까지 포함한 한 달 전체 식비예요. 위 장보기 예산과 별도로 관리해요.':'한 달 동안 사용할 식비를 정해 주세요. 주간 한도는 필요할 때 따로 설정할 수 있어요.'}</p>
  {!data?<p role="status">예산 정보를 불러오는 중이에요.</p>:<>
   {[{action:'monthlyBudget',label:`${month?.replace('-','년 ')}월 식비 예산`,amount:data.monthlyBudget},{action:'budget',label:'이번 주 식비 예산',amount:data.budget}].filter(item=>!monthlyOnly||item.action==='monthlyBudget').map(item=>{const form=<form className="budget-settings-form" key={`${item.action}-${item.amount??'empty'}`} onSubmit={e=>save(e,item.action)}>
   ...
   {!monthlyOnly&&data.monthlyBudget!=null&&data.budget!=null&&data.budget>data.monthlyBudget&&<p className="body-note">주간 예산이 월 예산보다 높아요. 입력한 금액을 확인해 주세요.</p>}
  </>}
