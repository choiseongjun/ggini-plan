...
export function RecordEntry({userId,onLogin,onLogged}:{userId?:string;onLogin:()=>void;onLogged:()=>void}){
 ...
 return <section className="record-entry" aria-label="식사 기록하기">
  ...
  <div className="record-entry-actions"><button type="button" aria-pressed={mode==='photo'} onClick={()=>choose('photo')}>사진으로 기록</button><button type="button" aria-pressed={mode==='search'} onClick={()=>choose('search')}>음식 검색으로 기록</button></div>
  <small>사진 기록은 음식 선택 → 사진 분석 → 저장 순서예요.</small>
  <Link className="record-guide-link" href="/how-to#record">사진·식사 기록 방법 보기</Link>
