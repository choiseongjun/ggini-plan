...
export default function HowTo(){
 return <AppShell><header className="app-header"><Brand/><Link className="tutorial-home" href="/">홈으로</Link></header><div className="app-content tutorial-content">
  <section className="tutorial-welcome"><div><span>처음 만나는 끼니플랜</span><h1>한 끼부터,<br/>같이 시작해요.</h1><p>끼니플랜은 메뉴 고민을 덜고, 먹은 한 끼를 기록하며 식사 친구 끼니와 함께하는 서비스예요.</p></div><RiceBuddy/></section>
  <Link className="tutorial-live-start" href="/?guide=start">화면에서 직접 따라 해보기</Link>
  <section className="tutorial-map" aria-label="세 가지 메뉴 안내"><h2>이 세 곳만 기억하세요</h2><Link href="/"><strong>홈</strong><span>먹을 메뉴를 추천받고 바꿔요</span></Link><Link href="/record"><strong>기록</strong><span>먹은 음식을 남기고 지난 식사를 봐요</span></Link><Link href="/profile"><strong>마이</strong><span>내 정보·식사 알림을 설정하고 끼니를 만나요</span></Link><p>추천 없이 기록부터 시작해도 괜찮아요. 아래에서 필요한 설명만 골라 보세요.</p></section>
  ...
  <section className="tutorial-step" id="recommend"><span className="tutorial-number">01</span><h2>먼저 식단을 추천받아요</h2><p>홈에서 <strong>‘내 식단 추천받기’</strong>를 누르세요. 현재 조건으로 추천된 메뉴를 확인하고, 마음에 들면 ‘이대로 먹기’로 저장하세요. 일정이 끝나면 ‘같은 조건으로 다음 식단 만들기’로 이어갈 수 있어요.</p><div className="tutorial-example"><span>처음이라면 이렇게</span><p>우선 추천 메뉴를 살펴보고, 조건을 바꾸고 싶을 때 상세설정을 열어보세요.</p></div><Link href="/">홈에서 추천받기</Link></section>
  <section className="tutorial-step" id="customize"><span className="tutorial-number">02</span><h2>내 조건으로 추천받고 싶다면</h2><p>홈의 <strong>‘취향·못 먹는 재료 설정’</strong>을 누른 뒤 <strong>‘더 설정하기’</strong>를 펼치세요.</p><ul><li><strong>끼니 수와 시간</strong> — 몇 끼를 준비할지, 아침·점심·저녁 중 어떤 끼니를 챙길지 선택해요.</li><li><strong>인원과 재료비</strong> — 함께 먹을 사람 수와 재료비 조건을 정해요.</li><li><strong>피할 재료</strong> — 못 먹거나 피하고 싶은 재료를 선택해요.</li></ul><p>설정 후 <strong>‘이 조건으로 추천받기’</strong>를 누르면 돼요. 신체 정보와 영양 목표는 마이페이지 상단의 ‘내 정보 수정’에서 설정할 수 있어요.</p><Link href="/profile#profile-settings">마이페이지에서 내 정보 설정하기</Link></section>
  ...
  <section className="tutorial-step" id="record"><span className="tutorial-number">04</span><h2>먹은 한 끼를 기록해요</h2><p>하단의 기록 탭에서 음식 검색 또는 사진으로 기록을 시작하세요. 추천·구매 등록 없이도 가능해요. 사진만 올리면 음식과 먹은 양, 예상 영양정보를 분석해요. 추천 메뉴와 다른 음식을 먹어도 기록할 수 있어요.</p><p>기록 탭에서 날짜를 골라 지난 식사를 보고, 양 수정·삭제도 할 수 있어요. 사진은 용량을 줄여 식사 내역과 함께 저장돼요. 내 일기에서 다시 보고, 기록을 삭제하면 사진도 함께 삭제돼요. 사진 기록은 화면의 전송·분석 안내를 확인한 뒤 진행해 주세요. 기록이 쌓이면 마이페이지의 <strong>주간 리포트</strong>에서 식사와 영양을 돌아볼 수 있어요.</p><Link href="/record">식사 기록하러 가기</Link></section>
