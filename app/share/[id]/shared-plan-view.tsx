...
export function SharedPlanView({id}:{id:string}){
 ...
 return <main className="shared-plan-page"><Link className="shared-brand" href="/">🍚 끼니플랜</Link>{auth?<AuthScreen onExplore={()=>setAuth(false)} onSuccess={user=>{setUserId(user.id);setAuth(false);setMessage('로그인했어요. 예산을 확인하고 이 식단으로 시작해 보세요.');}}/>:<>
  ...
  {plan&&<>
   <section className="shared-total"><span>{plan.days}일 · {plan.meals.length}끼 전체 구매 예상</span><strong>{won(plan.total)}</strong><p>공유 당시 판매 묶음 가격 기준 · 배송비 별도<br/>보유 재료 없이 모두 구매할 때의 금액이에요.</p></section>
   ...
   {Array.from({length:plan.days},(_,i)=>i+1).map(day=><section className="shared-day" key={day}><h2>🌱 {day}일차</h2>{plan.meals.filter(m=>m.day===day).map((meal,index)=>{const p=plan.products.find(p=>p.id===meal.productId)!;return <article key={index}><ProductThumb item={{productImageUrl:p.image,emoji:'🍽️',color:''}}/><div><small>{slotLabels[meal.slot]} · 1회분</small><h3>{p.name}</h3><strong>한 끼 약 {won(p.price/p.servings)}</strong><p>{p.calories===null?'칼로리 미확인':`${Math.round(p.calories)} kcal`} · 단백질 {p.protein===null?'미확인':`${p.protein} g`}</p>{p.url&&<a href={p.url} target="_blank" rel="noopener noreferrer">판매처에서 보기 ↗</a>}</div></article>;})}</section>)}
   <details className="shared-packs"><summary>전체 구매 수량 보기</summary>{plan.products.map(p=><p key={p.id}>{p.name} · {p.packs}묶음 · {won(p.price*p.packs)}</p>)}</details>
