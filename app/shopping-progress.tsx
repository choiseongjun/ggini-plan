...
export function useShoppingProgress(userId:string|undefined,scope:'products'|'ingredients'){
 ...
 async function persist(makeStock:()=>ShoppingStock,resetConditions?:PlanConditions){
  ...
  try{
   ...
   if(userId){const r=await fetch(endpoint,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({stock:next,version,resetConditions})});const d=await r.json();if(!r.ok){setReady(false);throw new Error(d.error);}data=d;}
   ...
   if(resetConditions){const draftKey=`kkiniplan-shopping-draft-v2-${userId??'guest'}`;localStorage.setItem(draftKey,JSON.stringify({conditions:resetConditions,mealIds:[]}));sessionStorage.removeItem(draftKey);}
 ...
 return {scope,stock,ready,busy,error,update:(changes:StockChange[],action:StockAction)=>persist(()=>changeStock(stock,changes,action)),reset:(conditions:PlanConditions)=>persist(()=>({}),conditions),reload:()=>{setReady(false);setError('');setRevision(n=>n+1);}};
}
...
export function ShoppingProgress({items,progress,guest=false,recommended=false,single=false,summary,heading}:{items:PurchaseItem[];progress:Progress;guest?:boolean;recommended?:boolean;single?:boolean;summary?:ReactNode;heading?:ReactNode}){
 ...
 return <section className={`purchase-progress${recommended?' purchase-recommended':''}`} aria-label="구매와 보유 상태 관리">
  ...
  <nav className="purchase-tabs" aria-label="구매 상태">{tabs.map(t=><button type="button" key={t} aria-pressed={tab===t} onClick={()=>{setTab(t);setSelected(single?items.map(i=>i.id):[]);setCheckout(false);setQuantities({});setMessage('');}}>{labels[t]} <b>{count(t)}</b></button>)}</nav>
  ...
  {ready&&<>
   {!rows.length?<p className="purchase-empty">{buying?'지금 추가로 살 것이 없어요. 주문·보유 목록을 확인해 주세요.':tab==='ordered'?'아직 주문 표시한 상품이 없어요.':'받았어요 또는 이미 있어요로 보유 재료를 등록해 보세요.'}</p>:<>
    {!single&&<label className="purchase-select-all"><Checkbox disabled={busy||!selectable.length} checked={selectable.length>0&&chosen.length===selectable.length} onChange={e=>setSelected(e.target.checked?selectable.map(i=>i.id):[])}/>{recommended&&buying?'살 것 모두 선택':'전체 선택'} ({selectable.length})</label>}
    <div className="purchase-items">{rows.map(i=><article key={i.id}>
     <label className="purchase-item-heading">{!single&&<Checkbox disabled={busy||quantity(i)===0} checked={chosen.some(c=>c.id===i.id)} onChange={e=>setSelected(prev=>e.target.checked?[...prev,i.id]:prev.filter(id=>id!==i.id))}/>} {i.thumbnail}<strong>{i.name}</strong></label>
     {i.recommendation}
