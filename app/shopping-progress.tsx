...
export function ShoppingProgress({items,progress,guest=false}:{items:PurchaseItem[];progress:Progress;guest?:boolean}){
 ...
 const invalidQuantity=(i:PurchaseItem)=>!validStockQuantity(chosenQuantity(i))||chosenQuantity(i)<=0||chosenQuantity(i)>(tab==='buy'?10000000:quantity(i));
  ...
  {ready&&<>
   {!rows.length?<p className="purchase-empty">{tab==='buy'?'지금 추가로 살 것이 없어요. 주문·보유 목록을 확인해 주세요.':tab==='ordered'?'아직 주문 표시한 상품이 없어요.':'받았어요 또는 이미 있어요로 보유 재료를 등록해 보세요.'}</p>:<>
    ...
    <div className="purchase-items">{rows.map(i=><article key={i.id}>
     ...
     {selected.includes(i.id)&&<label className="purchase-quantity">처리할 수량<input aria-label={`${i.name} 처리할 수량`} type="number" min={0.000001} max={tab==='buy'?10000000:quantity(i)} step="any" disabled={busy} value={quantities[i.id]??defaultQuantity(i)} onChange={e=>setQuantities(v=>({...v,[i.id]:Number(e.target.value)}))}/>{i.unit}</label>}
    </article>)}</div>
