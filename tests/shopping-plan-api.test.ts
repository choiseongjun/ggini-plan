...
test('guest recommendation, authenticated save, account isolation and server validation',async()=>{
 ...
 try{
  ...
  const fullCatalog=await loadPlanCatalog(ids[0]);
  const restoredProduct=slotCandidates(fullCatalog.products,cleared,0).find(p=>!allowsExcludedFoods(p,['shrimp'])&&!/시리얼|그래놀라/.test(p.name));
  assert.ok(restoredProduct);
  const selected=await loadPlanCatalog(ids[0],[restoredProduct.id]);
  assert.deepEqual(selected.products,fullCatalog.products.filter(p=>p.id===restoredProduct.id));
  assert.equal(personalized.personalization.nutritionMatched,null);
