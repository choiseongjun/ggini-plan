...
- 사진 인식 단계는 여러 구성품 표를 합산하지 않으며, 누락되거나 불명확한 값은 null로 남깁니다.
- 수집 화면의 `영양표에 없는 값은 AI 추정으로 채우기`가 켜져 있으면 모든 사진을 확인한 뒤 없는 항목만 별도로 추정합니다. 확인된 숫자는 유지하고 같은 기준량으로 추정합니다. 기준량도 없으면 판매 상태 100g 기준으로 추정합니다.
- 추정 항목·가정·모델·시각은 `nutrition_estimate`에 저장되고 관리자·상품 화면에 표시됩니다. 추정치는 제조사 확인값이 아닙니다.
