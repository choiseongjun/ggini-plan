...
기존 데이터는 KR/KRW/ko-KR/Asia/Seoul로 보존하며 금액 숫자를 바꾸지 않는다. **2026-09-17 개발·운영 공용 Supabase에 적용 완료.** 사전 롤백 검증 후 트랜잭션으로 적용했으며, 상품 500개와 기존 예산·섭취 기록·식단의 건수 및 금액 합계가 유지되는 것을 확인했다. 운영 API에서 KR 상품 500개, JP 상품 0개 및 준비 상태를 확인했다. 격리된 테스트 스키마에서도 기존 데이터 보존 및 반복 적용을 검증했다.
