...
식비·식단·장바구니를 한 화면에서 보는 앱입니다. 로그인 없이 홈과 달력, 장바구니, 기록을 둘러볼 수 있습니다. 계정과 로그인 세션은 `.env.local`의 `DATABASE_URL`이 가리키는 PostgreSQL에 저장합니다.

## 로컬 실행

로컬 설정은 Git에서 제외된 `.env.local` 한 파일에 보관합니다. `.env.example`을 참고해 연결값을 채운 뒤 실행하세요.
...
Docker Desktop으로 로컬 DB를 쓸 경우 `npm run db:setup`을 실행하세요. 이 명령은 로컬 DB 비밀번호를 `.env.local`에 저장하고, PostgreSQL 컨테이너와 로컬 DB 테이블을 만듭니다. 기존 `DATABASE_URL`이 원격 DB를 가리키면 변경하지 않으므로, 앱에서도 로컬 DB를 쓰려면 `DATABASE_URL_LOCAL` 값을 `DATABASE_URL`에 지정해야 합니다.
...
Docker 로컬 DB는 `127.0.0.1:5432`에만 연결됩니다. 마이페이지의 키·체중·나이·계산식 성별·활동량·끼니 수는 로그인한 계정의 `body_profiles`에 저장합니다. 식단·예산·지출 기록의 계정별 저장은 아직 연결되지 않았습니다.
