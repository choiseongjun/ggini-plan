...
export const collectionCategories={tofu:'두부·콩',vegetables:'채소·버섯',fruit:'과일',grain:'쌀·잡곡',meat:'정육·달걀',seafood:'수산물',dairy:'유제품',kimchi:'김치·반찬',ready:'간편식·도시락'} as const;
...
export function collectionInput(value:unknown){
 ...
 if(!v||!['kurly','oasis'].includes(String(v.seller))||!Object.hasOwn(collectionCategories,String(v.category))||!Number.isInteger(v.count)||Number(v.count)<1||Number(v.count)>2000||!Number.isInteger(v.offset)||Number(v.offset)<0||Number(v.offset)>=Number(v.count))return null;
