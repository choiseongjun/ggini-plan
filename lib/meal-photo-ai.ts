...
// reliable than estimating calories from a photo from scratch. The photo is analysed and discarded.
...
const schema={
 ...
 required:['match','portion','extras','note'],
};
...
export async function analyzeMealPhoto(input:{images:Buffer[];dishName:string;ingredients:string[]},fetcher:typeof fetch=fetch){
 ...
 try{
  response=await fetcher('https://api.openai.com/v1/responses',{
   ...
   body:JSON.stringify({model,store:false,instructions,input:[{role:'user',content:[{type:'input_text',text},...images.map(image=>({type:'input_image',image_url:`data:image/jpeg;base64,${image.toString('base64')}`,detail:'low'}))]}],max_output_tokens:300,text:{format:{type:'json_schema',name:'meal_photo',strict:true,schema}}}),
  });
 ...
 let value:{match?:unknown;portion?:unknown;extras?:unknown;note?:unknown};
 ...
 return {match,portion,extras,note,model};
}
