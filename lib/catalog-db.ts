...
export async function catalogItems(): Promise<CatalogItem[]> {
  const result = await getPool().query<CatalogRow>(`SELECT category,in_weekly_cart,product_image_url,unit,emoji,color,price_checked_at,price_note,allergens,id, name, detail, price, portions, quantity, search_query, product_url,
    nutrition_source_name, nutrition_source_url, nutrition_basis, calories_kcal, protein_g,
    carbohydrates_g, fat_g, sodium_mg, updated_at, nutrition_photo IS NOT NULL AS has_photo
  ...
  return result.rows.map((row) => ({
    ...
    searchQuery:row.search_query,productUrl:row.product_url,nutritionSourceName:row.nutrition_source_name,
    nutritionSourceUrl:row.nutrition_source_url,nutritionPhotoUrl:row.has_photo?`/api/catalog/photo?item=${encodeURIComponent(row.id)}`:null,
