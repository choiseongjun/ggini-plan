...
async function loadCatalogItems(context:MarketContext=korea): Promise<CatalogItem[]> {
  ...
  return result.rows.map((row) => ({
    ...
    nutritionEstimate:row.nutrition_estimate,nutritionBasis:row.nutrition_estimate ? `${row.nutrition_basis?.replace(/ · 추정 포함/g,'')??''} · 추정 포함` : row.nutrition_basis,caloriesKcal:row.calories_kcal===null?null:Number(row.calories_kcal),
