...
export function remainingPlanPortions(entries:{id:string;date:string}[],today:string,logs:{productId:string;portions:number}[],people=1){
 ...
 for(const {id,date} of entries){
  ...
  remaining[id]=(remaining[id]??0)+people-used;
 }
 return remaining;
}
