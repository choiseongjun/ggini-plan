...
export const mealKinds={rice:{label:'밥류',description:'볶음밥·덮밥·도시락·죽',emoji:'🍚'},noodles:{label:'면류',description:'국수·우동·파스타',emoji:'🍜'},bread:{label:'빵류',description:'샌드위치·토스트·베이글',emoji:'🍞'},light:{label:'가벼운 식사',description:'샐러드·시리얼·요거트',emoji:'🥗'}} as const;
export type MealKind=keyof typeof mealKinds;
export function validMealKinds(value:unknown):value is MealKind[]{return Array.isArray(value)&&value.length<=4&&new Set(value).size===value.length&&value.every(k=>typeof k==='string'&&Object.hasOwn(mealKinds,k));}
...
export function allowsMealKind(p:PlanProduct,selected?:MealKind[]){return !selected?.length||selected.includes(mealKind(p) as MealKind);}
