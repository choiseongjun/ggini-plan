...
export type DashboardData={isSample?:boolean;today:string;week:string;budget:number|null;expenses:{date:string;category:keyof typeof expenseCategories;amount:number}[];plans:{id:string;date:string;recommendation:NonNullable<ReturnType<typeof recommendMeals>>}[]};
