import {basket,mealSchedule,parseConditions,type PlanProduct,type MealSlot} from './shopping-plan';
import {servingNutrition} from './food-intake';

export type SharedPlan={days:number;slots:MealSlot[];meals:{productId:string;day:number;slot:MealSlot}[];products:{id:string;name:string;image:string|null;url:string|null;price:number;servings:number;calories:number|null;protein:number|null;packs:number}[];total:number};
