...
export type FoodDeal={id:string;title:string;food_type:string;source_name:string;source_url:string;product_url:string;price:number;shipping:number|null;pack:string;conditions:string;product_id:string|null;status:'draft'|'live'|'ended';checked_at:string|null;ends_at:string|null;updated_at:string};
...
export function dealState(deal:Pick<FoodDeal,'status'|'checked_at'|'ends_at'>,now=Date.now()){
 if(deal.status==='draft')return 'draft';
 if(deal.status==='ended'||(deal.ends_at&&Date.parse(deal.ends_at)<=now))return 'ended';
 if(!deal.checked_at||!Number.isFinite(Date.parse(deal.checked_at))||now-Date.parse(deal.checked_at)>86400000)return 'stale';
 return 'live';
}
