...
export function parseShoppingExpense(value:unknown):ShoppingExpense|null{
 ...
 if(typeof p.id!=='string'||!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(p.id)||!validPlanDate(p.date)||!Number.isSafeInteger(p.amount)||p.amount<0||p.amount>10000000||!['buy','order','backfill'].includes(p.action)||!Array.isArray(p.itemIds)||!p.itemIds.length||p.itemIds.length>300||p.itemIds.some(id=>!validStockId(id))||new Set(p.itemIds).size!==p.itemIds.length)return null;
