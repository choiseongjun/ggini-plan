import {taiwanFoodKind,taiwanFoodGuides,type TaiwanFoodKind} from './taiwan-food-types';
...
export const getTaiwanCatalog=cache(async()=>{
 ...
 return items.map(item=>({ ...item,foodKind:taiwanFoodKind(item.name,details.find(d=>d.product_id===item.id)?.category_path??[]),source:details.find(d=>d.product_id===item.id),servings:profiles.find(p=>p.product_id===item.id)?.servings??null}));
});
export function twMetadata(title:string,description:string,path:string):Metadata{return {title:{absolute:title+' | GginiPlan 台灣'},description,alternates:{canonical:siteUrl+path},openGraph:{title,description,locale:'zh_TW',url:siteUrl+path,siteName:'GginiPlan 台灣',type:'website',images:[socialImage('tw')]},twitter:{card:'summary_large_image',title,description,images:[socialImage('tw')]}};}
export const safeJson=(value:unknown)=>JSON.stringify(value).replaceAll('<','\\u003c');

export type TaiwanCatalogProduct=Awaited<ReturnType<typeof getTaiwanCatalog>>[number];
export async function taiwanFoodGroups(){const items=await getTaiwanCatalog();return (Object.keys(taiwanFoodGuides) as TaiwanFoodKind[]).map(key=>({key,label:taiwanFoodGuides[key][0],count:items.filter(p=>p.foodKind===key).length})).filter(g=>g.count>0);}
